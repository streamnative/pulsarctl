...
func TestFailurePersistence(t *testing.T) {
	...
	assert.Equal(t, execErr.Error(), "code: 400 reason: Bookkeeper Ensemble (2) >= WriteQuorum (5) >= AckQuorum (2)")
	...
	_, execErr, _, _ = TestNamespaceCommands(setPersistence, setArgs)
	assert.NotNil(t, execErr)
	assert.Equal(t, execErr.Error(), "code: 400 reason: Bookkeeper Ensemble (2) >= WriteQuorum (2) >= AckQuorum (3)")
}
