...
func GetRetentionCmd(vc *cmdutils.VerbCmd) {
	...
	desc.CommandUsedFor = "Get the retention policy of a topic"
	desc.CommandPermission = "This command requires tenant admin permissions."

	var examples []cmdutils.Example
	getRetention := cmdutils.Example{
		Desc:    "Get the retention policy of a topic",
	...
	vc.SetDescription(
		"get-retention",
		"Get the retention policy of a topic",
		desc.ToString(),
...
func doGetRetention(vc *cmdutils.VerbCmd) error {
	...
	policy, err := admin.Topics().GetRetention(*topicName)
	if err == nil {
