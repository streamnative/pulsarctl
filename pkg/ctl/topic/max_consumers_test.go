...
func TestMaxConsumers(t *testing.T) {
	...
	topicName := "persistent://public/default/test-max-consumers-topic" + test.RandomSuffix()
	...
	g.Eventually(func(g gomega.Gomega) {
		...
		g.Expect(setOut.String()).Should(
			gomega.Equal("Set max number of consumers successfully for [" + topicName + "]\n"))
	...
	removeArgs := []string{"remove-max-consumers", topicName}
	g.Eventually(func(g gomega.Gomega) {
		removeOut, execErr, _, _ := TestTopicCommands(RemoveMaxConsumersCmd, removeArgs)
		g.Expect(execErr).Should(gomega.BeNil())
		g.Expect(removeOut.String()).Should(
			gomega.Equal("Remove max number of consumers successfully for [" + topicName + "]\n"))
	}).Should(gomega.Succeed())
	...
	g.Eventually(func(g gomega.Gomega) {
		...
		g.Expect(execErr.Error()).Should(
			gomega.Equal("code: 412 reason: maxConsumers must be 0 or more"))
	}).Should(gomega.Succeed())
}
