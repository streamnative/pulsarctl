...
func TestMaxUnackMessagesPerSubscription(t *testing.T) {
	...
	g.Eventually(func(g gomega.Gomega) {
		...
		g.Expect(setOut.String()).Should(
			gomega.Equal("Set max unacked messages per subscription successfully for [" + topicName + "]\n"))
	...
	removeArgs := []string{"remove-max-unacked-messages-per-subscription", topicName}
	g.Expect(func(g gomega.Gomega) {
		removeOut, execErr, _, _ := TestTopicCommands(RemoveMaxUnackMessagesPerSubscriptionCmd, removeArgs)
		g.Expect(execErr).Should(gomega.BeNil())
		g.Expect(removeOut.String()).Should(
			gomega.Equal("Remove max unacked messages per subscription successfully for [" + topicName + "]\n"))
	}).Should(gomega.Succeed())
