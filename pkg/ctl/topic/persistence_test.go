...
func TestPersistence(t *testing.T) {
	...
	g.Expect(execErr).Should(gomega.BeNil())
	g.Expect(setOut.String()).Should(gomega.Equal("Set persistence successfully for [" + topicName + "]\n"))
	...
	g.Eventually(func(g gomega.Gomega) {
		...
		g.Expect(persistenceData.ManagedLedgerMaxMarkDeleteRate).Should(gomega.Equal(float64(2.2)))
	}).Should(gomega.Succeed())

	setArgs = []string{"remove-persistence", topicName}
	setOut, execErr, _, _ = TestTopicCommands(RemovePersistenceCmd, setArgs)
	g.Expect(execErr).Should(gomega.BeNil())
	g.Expect(setOut.String()).Should(gomega.Equal("Remove persistence successfully for [" + topicName + "]\n"))
	...
	g.Expect(execErr.Error()).Should(gomega.Equal("code: 400 reason: Bookkeeper " +
		"Ensemble (1) >= WriteQuorum (4) >= AckQuorum (3)"))
}
