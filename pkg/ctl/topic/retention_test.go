...
func TestRetentionCmd(t *testing.T) {
	...
	g.Eventually(func(g gomega.Gomega) {
		out, execErr, _, _ := TestTopicCommands(SetRetentionCmd, args)
		g.Expect(execErr).Should(gomega.BeNil())
		g.Expect(out).ShouldNot(gomega.BeNil())
		g.Expect(out.String()).Should(gomega.Equal("Set the retention policy successfully on [%s]\n"))
	...
	g.Eventually(func(g gomega.Gomega) {
		out, execErr, _, _ := TestTopicCommands(RemoveRetentionCmd, args)
		g.Expect(execErr).Should(gomega.BeNil())
		g.Expect(out).ShouldNot(gomega.BeNil())
		g.Expect(out.String()).Should(gomega.ContainSubstring("successfully"))
	}).Should(gomega.Succeed())
