...
case ${TEST_ARGS} in
    token)
        ...
        go test -v -race ./pkg/auth/token.go ./pkg/auth/token_test.go
        ;;
    tls)
        echo "running tls tests"
        go test -v -race ./pkg/ctl/cluster -run TestTLS
        ;;
    function)
        ...
        go test -v -race -test.timeout 10m $(go list ./... | grep function)
        ;;
    sink)
        ...
        go test -v -race -test.timeout 10m ./pkg/ctl/sinks
        ;;
    source)
        ...
        go test -v -race -test.timeout 10m ./pkg/ctl/sources
        ;;
    packages)
        ...
        cp /pulsar/examples/api-examples.jar test/functions/
        cp /pulsar/examples/python-examples/logging_function.py test/functions/
        go test -v -race -test.timeout 10m $(go list ./... | grep packages)
        ;;
    *)
        echo "running normal unit tests"
        go test -v -race $(go list ./... | grep -v bookkeeper | grep -v bkctl | grep -v functions | grep -v sources | grep -v sinks | grep -v packages | grep -v test)
        ;;
esac
# stop pulsar service
