...
readonly PULSAR_DEFAULT_VERSION="2.11.0.0-rc2"
...
case ${1} in
    ...
    tls)
        ...
        ;;
    function)
        docker run --name function --rm -e PULSAR_STANDALONE_USE_ZOOKEEPER=1 -e TEST_ARGS=function -e FUNCTION_ENABLE=true ${IMAGE_NAME}
        ;;
    sink)
    ...
    source)
        ...
        ;;
    packages)
        docker run --name packages --rm -e PULSAR_STANDALONE_USE_ZOOKEEPER=1 -e TEST_ARGS=packages -e PULSAR_PREFIX_enablePackagesManagement=true -e PULSAR_PREFIX_zookeeperServers=127.0.0.1:2181 ${IMAGE_NAME}
        ;;
    *)
